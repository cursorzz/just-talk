...
推送与应用版本一致的 `v*` 标签（例如版本 `0.1.0` 对应 `v0.1.0`），会生成 macOS ARM64 DMG、Windows x64 NSIS、Linux x64 AppImage/DEB，并在三个构建全部成功后自动创建 GitHub Release。Release 同时包含所有安装包和 `SHA256SUMS.txt`。
